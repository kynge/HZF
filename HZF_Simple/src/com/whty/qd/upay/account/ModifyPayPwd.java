...
public class ModifyPayPwd extends BaseActivity {
	...
	protected static final String TAG = "ModifyPayPwd";
...
//							| InputType.TYPE_TEXT_VARIATION_PASSWORD);
	...
	protected boolean validate() {
		...
		if (newPwd.length() < 6 || newPwd.length() > 20) {
			Toast.makeText(mContext, R.string.input_pay_pwd, Toast.LENGTH_SHORT).show();
			return false;
		}
		...
		if(HomeUtils.userLoginPwdCache.equals(Constant.md5(newPwd))){
	...
	protected void sendRequest() {
		...
		params.add(new BasicNameValuePair("payPwd", Constant.md5(oldPwd)));
		params.add(new BasicNameValuePair("newPayPwd", Constant.md5(newPwd)));
		params.add(new BasicNameValuePair("para", "updatePayPwd"));
			...
			public void data(byte[] data, int length, String url,
				...
				try {
					...
					if (RetCode.OK.equals(retCode)) {
						...
						HomeUtils.userPayPwdCache =  Constant.md5(newPwd);
					} else {
